...
class Mentor:
    ...
    def __init__(
        ...
        self.preferred_weekdays = (
            preferred_weekdays if preferred_weekdays is not None else []
        )
...
class Day:
    ...
    def __init__(self, date_info: dt.datetime):
        ...
        self.mentors_on_shift: Dict[str, Mentor] = {
            shift: None for shift in self.shifts
        }
    ...
    def get_shifts(self, season: str, day: int) -> Dict[str, int]:
        ...
        # not using them.
        position = list(self.week_day_map.values()).index(
            day
        )  # get index of key of shift for given weekday
...
class Schedule:
    ...
    def __init__(self, year: int, month: int, len_p1: int):
        ...
        self.month = month
        self.year = year
        len_month = monthrange(year, month)[
            1
        ]  # get num days in month, used to calc len_p2
        self.m1 = self.create_mentor_info(len_p1, "<=", len_p1)
        self.pay1 = self.create_pay_days(
            self.m1, dt.datetime(year, month, 1), dt.datetime(year, month, len_p1)
        )
        ...
        self.pay2 = self.create_pay_days(
            self.m2,
            start_date=dt.datetime(year, month, len_month - (len_month - len_p1) + 1),
            end_date=dt.datetime(year, month, len_month),
            offset=len_p1,
        )
    ...
    def get_dates_of_weekday(self, day: str) -> List[int]:
        ...
        for i in range(7):  # 7 days in a week
            if (
                self.week_day_map[day]
                == dt.datetime(self.year, self.month, i + 1).weekday()
            ):
                idx = i + 1
                break
    ...
    def hard_date_adj(
        self, hard_dates: List[int], weekdays: List[str], behavior: str
    ) -> List[int]:
        if len(weekdays) == 0:
            return hard_dates
    ...
    def create_mentor_info(
        self, len_pay: int, comparator: str, end_day: int = 1
    ) -> List[Mentor]:
        ...
        for name, info in mentor_info.items():
            c_info = info.copy()
            new_dates = self.hard_date_adj(
                info["hard_dates"], info["weekdays"], info["weekday_behavior"]
            )
            c_info["hard_dates"] = [
                date for date in new_dates if get_truth(date, comparator, end_day)
            ]
            c_info["name"] = name
    ...
    def prioritize_days(self, pay_days: List[Day]):
        ...
        total_available_days = sum(
            [day.get_mentor_days() for day in pay_days]
        )  # total workable shifts
        for day in pay_days:
            # Normal priority:
        ...
        pay_days.sort(
            key=lambda day: (day.priority_value, -day.get_available_mentor_hours())
        )

    def filter_saturday_candidates(
        self, candidates: List[Mentor], current_day: Day, assigned_days: List[Day]
    ) -> List[Mentor]:
        ...
        mentors_last_saturday = {
            mentor
            for mentor in previous_saturday.mentors_on_shift.values()
            if mentor is not None
        }
        filtered = [
            mentor for mentor in candidates if mentor not in mentors_last_saturday
        ]
    ...
    def assign_shift(self, pay_days: List[Day]) -> Union[int, Mentor]:
        ...
        update_mentors = (
            True  # prevents double updating mentors available days on recursive calls
        )
        ...
        preferred_candidates = [
            ...
            if day_name in mentor.preferred_weekdays
        ]
        # If at least one mentor prefers this day, consider only those; otherwise use all available mentors.
        candidates = (
            preferred_candidates if preferred_candidates else day.potential_mentors
        )

        # Apply the Saturday filter if the day is Saturday.
        candidates = self.filter_saturday_candidates(
            candidates, day, self.assigned_days
        )
    ...
    def mentor_cleanup(
        self,
        mentor_update: Union[int, Mentor],
        pay_days: List[Day],
        mentors: List[Mentor],
    ):
        ...
        if type(mentor_update) is Mentor:  # we can just check passed mentor
            ...
            raise ValueError(
                "got bad datatype {0} must pas int or Mentor".format(
                    type(mentor_update)
                )
            )

        for day in pay_days:
            day.potential_mentors = [
                mentor
                for mentor in day.potential_mentors
                if mentor not in mentors_to_update
            ]
    ...
    def assign_all_shifts(self, pay_days: List[Day], mentors: List[Mentor]):
        ...
        self.assigned_days.sort(
            key=lambda day: (day.date_info.day)
